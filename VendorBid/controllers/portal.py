...
from odoo.exceptions import AccessDenied, ValidationError as OdooValidationError
import base64


class SuppliesPortal(CustomerPortal):
    def _binary_file_vals(self, file_storage):
        if not file_storage or not getattr(file_storage, 'filename', None):
            return False, False
        return base64.b64encode(file_storage.read()), file_storage.filename

    @http.route('/supplies/rfp/<string:rfp_number>', auth='public', website=True)
    def public_supplies_rfp(self, rfp_number, **kw):
        ...
        if not rfp:
            raise AccessDenied("This RFP is not publicly available.")
        return request.render('VendorBid.public_supplies_rfp_summary', {
            'rfp': rfp,
            'login_url': '/web/login?redirect=/my/supplies/%s' % rfp.rfp_number,
            'register_url': '/supplies/register',
        })
    ...
    def supplies_portal_rfp(self, rfp_number, **kw):
        ...
        if not rfp:
            raise AccessDenied("You do not have access to this RFP.")
        ...
        if request.httprequest.method == 'POST':
            try:
                ...
                if rfp.state != 'published':
                    raise AttributeError('This RFP is not open for bid submission.')
                if rfp.submission_deadline and rfp.submission_deadline <= fields.Datetime.now():
                    rfp.sudo().action_close()
                    raise AttributeError('The submission deadline has passed.')
                if rfp.visibility == 'private' and partner_id not in rfp.selected_vendor_ids:
                    raise AccessDenied("You are not authorized to bid on this private RFP.")
                ...
                try:
                    ...
                    file_map = {
                        'signed_quotation_doc': 'signed_quotation_filename',
                        'technical_proposal_doc': 'technical_proposal_filename',
                        'experience_evidence_doc': 'experience_evidence_filename',
                        'rfp_specific_doc': 'rfp_specific_filename',
                    }
                    doc_vals = {'bid_submitted_at': fields.Datetime.now()}
                    for field_name, filename_field in file_map.items():
                        payload, filename = self._binary_file_vals(request.httprequest.files.get(field_name))
                        if payload:
                            doc_vals[field_name] = payload
                            doc_vals[filename_field] = filename
                    rfq.write(doc_vals)
                    rfq._validate_required_bid_documents()
