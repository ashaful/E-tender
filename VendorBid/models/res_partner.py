...
class ResPartner(models.Model):
    ...
    vendor_document_complete = fields.Boolean(
        string='Required Vendor Documents Complete',
        compute='_compute_vendor_document_complete',
        store=False,
    )

    def _get_vendor_document_requirements(self):
        self.ensure_one()
        if not sql.table_exists(self.env.cr, 'supplies_document_requirement'):
            return self.env['supplies.document.requirement']
        if not self.product_category_id:
            return self.env['supplies.document.requirement']
        return self.env['supplies.document.requirement'].sudo().search([
            ('active', '=', True),
            ('required', '=', True),
            ('vendor_category_id', 'parent_of', self.product_category_id.id),
        ])

    def _get_missing_vendor_documents(self):
        self.ensure_one()
        return self._get_vendor_document_requirements()._get_missing_for_partner(self)

    def _compute_vendor_document_complete(self):
        for partner in self:
            partner.vendor_document_complete = not bool(partner._get_missing_vendor_documents())

    def action_validate_vendor_documents(self):
        for partner in self:
            missing = partner._get_missing_vendor_documents()
            if missing:
                raise ValidationError(_('Missing required vendor documents: %s') % ', '.join(missing))
        return True
